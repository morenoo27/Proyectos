...
function editar_grupos(dia, hora, id_usuario, aula, id_aula) {
    ...
    $.ajax({
        ...
        output += "<table id='tabla_editado' class='table table-responsive table-striped'>";
        ...
        if (aula == "Sin asignar o sin aula" && filas_tabla.length == 1) {
            ...
            output += "<select onchange='sin_aula(\"" + dia + "\", \"" + hora + "\")' name='grupos_libres' id='grupos_libres'>";
            ...
            output += "<label>Grupo:</label>";
            output += "<select onchange='sin_aula(\"" + dia + "\", \"" + hora + "\", \"" + id_aula + "\")' name='grupos_libres' id='grupos_libres'>";
...
function aniadir_grupo(dia, hora, id_usuario) {
    ...
    $.ajax({
        ...
        if (!data.profesor || id_usuario != data.profesor[0].usuario) {
            ...
            //console.log(data);
            if (data.ocupada && data.profesor[0].grupo != select_de_grupos[0]) {
                ...
                if (data.profesor.length == 1) {
                    ...
                    console.log(copia_array_prof);
                    console.log(copia_array_grupo);

                    for (let i = 0; i < copia_array_prof.length; i++) {

                        html_code += "profesor " + copia_array_prof[i] + ", ";
                    }

                    if (copia_array_grupo.length == 1) {

                        html_code += " en el grupo " + copia_array_grupo[0] + "</p>";
                    } else {
                    ...
                    if (copia_array_grupo.length == 1) {
                        ...
                        html_code += "<p class='centrar'>Para añadir este aula a " + select_de_grupos[1] + ", debes cambiarle antes el aula a " + copia_array_grupo[0]; + "</p>";
                    } else {

                        html_code += "<p class='centrar'>Para añadir este aula a " + select_de_grupos[1] + ", debes cambiarle antes el aula a " + copia_array_grupo[0] + ", ";
                ...
                html_code += "<p class='centrar'><button onclick='cerrar_modal();'>Cancelar</button> <button onclick='selec_aula(\"" + dia + "\", \"" + hora + "\",\"" + profesores + "\",\"" + id_usuario + "\",\"" + data.profesor[0].nombre_grupo + "\");event.preventDefault();'>Cambiar</button></p>";

                //console.log(profesores);
                //console.log(id_usuario);
                //console.log(profesores.includes(id_usuario));
                let flag = false;
...
function confirmar_cambio_aula(dia, hora, id_usuarios_antiguos, id_usuario) {
    ...
    console.log(usuarios_antiguos);
    console.log(id_usuario);

    //if (usuarios_antiguos.includes(id_usuario))
